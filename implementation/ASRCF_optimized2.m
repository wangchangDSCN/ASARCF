...
for frame = 1:num_frames
  ...
  tic();  
    ...
    %learning_rate 
  A1=size(im,1);A2=size(im,2);
    data{frame}=im;
    if frame > 1
        diff = data{1, frame} - data{1, frame-1};
        Edis = sqrt(sum(diff(:).*diff(:)));
        E=Edis/(A2*A1);
        learning_rate = Q * E + e;
    end
...
%% main loop
    if frame > 1
       imim=im;
        new_g_f2 = g_f(:,:,1);
        real_g2 = real(new_g_f2);
        if(all(isnan(real_g2(:,1))))
            g_f = g_f_old;
        end
...
%         end
 figure(3)
%             subplot(121)
%             imim=insertShape(imim,'Rectangle',box);
%             imshow(imim)
%             x = ['frame',num2str(frame)];
%             title(x)
% %             
%             subplot(122)
%             imshow(fftshift(response1))
...
%             
         ...
         [ss,yy,img] = get_subwindow_no_window(pixels,(use_sz*2),(use_sz));
        imshow(img);
        if size(img,3)==3
...
% ADMM solution    
   ...
   % ADMM solution for localization
    ...
    new_g_f = g_f(:,:,1);
    real_g = real(new_g_f);
    if(~all(isnan(real_g(:,1))))
        g_f_old = g_f;
    end
    
    
    for iteration = 1:params.al_iteration-1
        [w]=ADMM_solve_w2(params,use_sz,model_w,h_f);
        [g_f,h_f]=ADMM_solve_h(params,use_sz,model_xf,yf,small_filter_sz,w,model_w,frame);
    end
